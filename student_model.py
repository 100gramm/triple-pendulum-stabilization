...
class VisionExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict):
        ...
        self.vision_model = shufflenet_v2_x1_0(weights=None)
        self.vision_model.fc = nn.Sequential(
            nn.Linear(self.vision_model.fc.in_features, 512),
            nn.GELU(),
            nn.Dropout(0.2),
            nn.Linear(512, 14)
        )
        ...
        self.vision_model.load_state_dict(weights)
    ...
    def forward(self, observations: dict) -> torch.Tensor:
        ...
        with torch.no_grad():
            vision_features = self.vision_model(pixels)
            
        return torch.cat([vision_features, observations['target']], dim=1)


class TriplePendulumStudent(MujocoEnv):
    def __init__(self, xml_path=XML_PATH, targets=possible_targets, render_mode='rgb_array'):
        ...
        self.observation_space = Dict({
            'pixels': Box(low=0, high=255, shape=(3, IMG_SIZE, IMG_SIZE), dtype=np.uint8),
    ...
    def _get_obs(self) -> dict:
        ...
        return {"pixels": frame, "target": self.current_target.astype(np.float32)}
...
def make_env():
    def _init():
        env = TriplePendulumStudent()
        env = TimeLimit(env, max_episode_steps=500)
        return Monitor(env)
    return _init

LOG_DIR  = "./logs_student/"
MODEL_DIR = "./models_student/"
...
def train():
    ...
    print(f"Training Student on {model.device}...")
    model.learn(
        total_timesteps=TOTAL_TIMESTEPS, 
        callback=[checkpoint_callback, curriculum_callback, eval_callback],
        progress_bar=True
    )
    model.save(f"{MODEL_DIR}/student_final_model")
